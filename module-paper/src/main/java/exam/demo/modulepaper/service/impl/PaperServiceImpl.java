...
public class PaperServiceImpl extends ServiceImpl<PaperMapper, Paper> implements IPaperService {
    ...
    public Map<String, Object> queryPaper(PaperQueryDto paperQueryDTO, boolean isTemplate) {
        ...
        if (isTemplate) {
            ...
            wrapper.eq(MagicPointConstant.TEMPLATE, 0);
            if (userPermission.getCompanyId() != null) {
                wrapper.eq(MagicPointConstant.COMPANY_ID, userPermission.getCompanyId());
            } else {
                wrapper.eq(MagicPointConstant.ORG_ID, TokenUtils.getUser().getOrgId());
            }
        }
        if (paperQueryDTO.getDifficulty() != null) {
            wrapper.eq(MagicPointConstant.DIFFICULTY, paperQueryDTO.getDifficulty());
        }
        if (StringUtils.isNotBlank(paperQueryDTO.getCreatedBy())) {
            wrapper.likeRight(MagicPointConstant.PAPER_CREATOR, paperQueryDTO.getCreatedBy());
        }
        if (paperQueryDTO.getStart() != null && paperQueryDTO.getEnd() != null) {
            wrapper.between(MagicPointConstant.COMB_EXAM_TIME, paperQueryDTO.getStart(), paperQueryDTO.getEnd());
        }
    ...
    public List<PaperIdWithName> listByName(FuzzySearch fuzzySearch) {
        ...
        if (fuzzySearch.getCompanyId() != null) {
            wrapper.eq(MagicPointConstant.COMPANY_ID, fuzzySearch.getCompanyId());
        }
        if (fuzzySearch.getPaperName() != null) {
            wrapper.likeRight(MagicPointConstant.NAME, fuzzySearch.getPaperName());
        }
        List<Paper> paperList = list(wrapper);
        return paperList.stream().map(p ->
                new PaperIdWithName(p.getId(), p.getName())
